...
def test_graphsage_ppi_torch_trainer(mock_graph):
    ...
    run_args = f"""--data_dir {data_dir} --mode train --trainer base --train_workers 1 --seed 123 \
--backend snark --graph_type local --converter skip \
...
def test_graphsage_ppi_ddp_trainer(mock_graph):
    torch.manual_seed(0)
    np.random.seed(0)
    data_dir = "/tmp/ppi"
    num_nodes = 56944
...
def test_graphsage_ppi_hvd_trainer(mock_graph):
    torch.manual_seed(0)
    np.random.seed(0)
    data_dir = "/tmp/ppi"
    num_nodes = 56944
